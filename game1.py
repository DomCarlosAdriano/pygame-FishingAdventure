...
class Hero(Actor):
    ...
    def update(self):
        ...
        if keyboard.space and on_ground:
            if sound_on: sounds.jump.play() # <-- SOM DE PULO ADICIONADO
...
def update():
    ...
    for enemy in enemies:
        ...
        if enemy.check_collision(hero):
            if sound_on: sounds.hit.play() # <-- SOM DE PERDA/HIT ADICIONADO
            reset_game()
            game_started_message = " "
            return
    ...
    for coin in coins:
        ...
        if coin.check_collision(hero) and not coin.collected:
            if sound_on: sounds.coin.play() # <-- SOM DE MOEDA ADICIONADO
            coin.collected = True
            score += 10
